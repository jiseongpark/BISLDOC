...
from ..models import Hospital, Device, Care
...
def map(request):
    ...
    if kw:
        ...
        device = device.filter(encoded_id=hospital_id)  # QuerySet of devices for each hospital
        care = care.filter(encoded_id=hospital_id)  # QuerySet of cares for each hospital
        ...
        # Care
        care_list = [c.care for c in care]
        ...
        context = {
            ...
            'total_device_num': sum(device_num_list),
            'care_list': care_list,
            'kw': kw}
